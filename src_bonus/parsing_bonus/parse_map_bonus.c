...
/*   Updated: 2025/09/16 15:29:31 by oait-si-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_cub3d_map(t_config *config, t_gc *gc, const char *filename)
{
	...
	if (ft_strncmp(filename + (ft_strlen(filename) - 4), ".cub", 4) || is_hidden(filename + (ft_strlen(filename) - 4)))
		return (print_err("File error: invalid "
				"file; need filename.cub extension file\n"));
